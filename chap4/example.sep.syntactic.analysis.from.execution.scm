...
(define (analyze-filter expr)
  (let ((proc (analyze (cadr expr)))
    ...
    (lambda (env) (filter (lambda (item) (execute-application (proc env) (item env))) fval))))
(define (analyze-map expr)
  (let ((proc (analyze (cdr expr)))
        (mval (map analyze (cddr expr))))
    (lambda (env) (map (lambda (item) (execute-application (proc env) (item env))) mval))))
(define (analyze-flatmap expr)
  (let ((proc (analyze (cdr expr)))
        (fval (map analyze (cddr expr))))
    (lambda (env) (flatmap (lambda (item) (execute-application (proc env) (item env))) fval))))
    ...
    (if (null? and-sequence)
        ...
        (let ((sequence (map analyze and-sequence)))
          (define (analyze-and-sequence env seq)
            ((if (null? seq)
                 #t
                 (and ((car seq) env) (analyze-and-sequence env (cdr seq))))))
